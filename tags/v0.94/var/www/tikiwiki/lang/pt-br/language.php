...
"Attention" => "Atenção",
"Apache has the right to write in your file tree, which enables the installation, removal or \nupgrade of packages. When you are done with those operations, think to fix those permissions back to a safe state (by using \n\"./fixperms fix\" for example)." => "O Apache tem permissão para escrever na sua árvore de arquivos, o que permite a instalação, remoção ou atualização dos pacotes. Quando você terminar essas operações, é recomendável voltar as permissões para o modo mais seguro (usando \n\"./fixperms fix\" por exemplo).",
"To install, remove or upgrade packages you need to give the apache user the right\nto write files in your web tree (you can use \"./fixperms.sh open\" to set it up). After installation you need to remove that\npermission (using \"./fixperms fix\")." => "Para instalar, remover ou atualizar pacotes é necessário que o usuário do Apache tenha permissão de escrita na árvore de arquivos da web (você pode usar \"./fixperms.sh open\" para fazer isso). Depois da instalação você deve remover essa permissão (usando \"./fixperms fix\").",
