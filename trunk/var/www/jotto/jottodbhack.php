...
<h2>Place a record in the database with the magic name concatenated to your name and with a score of zero. You can get a hint as to that name by finding the worst possible jotto score.  If there is a tie find the older record.</h2>
...
<?php
   ...
   $result = mysql_query ("SELECT name,guess,count,tod FROM
                          results WHERE (count =0  AND name like  \"rot13%\") order by tod", $connection);
